...
readfile = open('input.txt').read().strip().split("\n")

def lmap(func, *iterables):
    return list(map(func, *iterables))

# 1D list for a 2d list
flatten = lambda x: [q for q in w for w in x]
...
neighbors_4d = [(x,y,z,w) for w in range(-1, 2) for z in range(-1, 2) for y in range(-1, 2) for x in range(-1, 2) if (x,y,z,w) != (0,0,0,0)]
